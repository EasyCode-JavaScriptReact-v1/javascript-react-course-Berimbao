...
PhoneApp.prototype.addUser = function(id,options){
  ...
  if(options.phone){
    if(!this.checkPhoneNumber(options.phone)){
      ...
      console.log('не удалось сохранить номер телефона, Телефон должен состоять только из цифр')
    } 
  }
...
PhoneApp.prototype.checkAndFormatPhoneNumber = (phone) => {
  if(!isNaN(+phone)){
    let resultPhone = '('+phone.slice(0,3)+') '+phone.slice(3,5)+"-"+phone.slice(5,7)+"-"+phone.slice(7);
    return resultPhone
  }
}
...
PhoneApp.prototype.editUser = function(id,options){
  this.database.map((elem) => {
    if(elem.id == id){
      if(options.name){elem.name = options.name}
      if(options.phone){elem.phone = options.phone}
      if(options.homePhone){elem.homePhone = options.homePhone}
    }
  })
}
...
myPhoneApp.addUser(1,{name:"Vasya",phone:'123456789',homePhone:'11111'});
...
myPhoneApp.addUser(5,{name:"Anduin",phone:'113456789',homePhone:'535353a'});
myPhoneApp.addUser(6,{name:"Torgrim",phone:'321456789'});
myPhoneApp.addUser(7,{name:"Anduin",phone:'113451189',homePhone:'222222'});
